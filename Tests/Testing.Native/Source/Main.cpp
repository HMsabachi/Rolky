...
static Rolky::String StringMarshalIcall(Rolky::String InStr)
{
	return InStr;
}
static void StringMarshalIcall2(Rolky::String InStr)
{
	std::cout << std::string(InStr) << std::endl;
}
static bool TypeMarshalIcall(Rolky::ReflectionType InReflectionType)
{
	Rolky::Type& type = InReflectionType;
...
static Rolky::Array<int32_t> EmptyArrayIcall()
{
	std::vector<int32_t> empty;
	return Rolky::Array<int32_t>::New(empty);
}

static Rolky::Array<float> FloatArrayIcall()
{
	std::vector<float> floats = { 5.0f, 10.0f, 15.0f, 50.0f };
	return Rolky::Array<float>::New(floats);
}

static Rolky::ManagedObject instance;
static Rolky::ManagedObject NativeInstanceIcall()
{
	return instance;
}

static void RegisterTestInternalCalls(Rolky::ManagedAssembly& InAssembly)
{
...
static void RegisterMemberMethodTests(Rolky::ManagedObject& InObject)
{
	...
	RegisterTest("BoolTest", [&InObject]() mutable { return InObject.InvokeMethod<Rolky::Bool32, bool>("BoolTest", false); });
	...
	{
		Rolky::ScopedString str = InObject.InvokeMethod<Rolky::String, Rolky::String>("StringTest", Rolky::String::New("Hello"));
...
static void RegisterFieldMarshalTests(Rolky::ManagedObject& InObject)
{
	...
	{
		auto value = InObject.GetPropertyValue<Rolky::Bool32>("BoolPropertyTest");
		if (value != false)
			return false;
		InObject.SetPropertyValue<Rolky::Bool32>("BoolPropertyTest", true);
		value = InObject.GetPropertyValue<Rolky::Bool32>("BoolPropertyTest");
		return static_cast<bool>(value);
	});
	RegisterTest("StringPropertyTest", [&InObject]() mutable
	{
		Rolky::ScopedString value = InObject.GetPropertyValue<Rolky::String>("StringPropertyTest");
		if (value != std::string_view("Hello"))
			return false;
		InObject.SetPropertyValue("StringPropertyTest", Rolky::String::New("Hello, World!"));
		value = InObject.GetPropertyValue<Rolky::String>("StringPropertyTest");
		return value == "Hello, World!";
	});
}
...
int main([[maybe_unused]] int argc, char** argv)
{
	...
	settings.RolkyDirectory = coralDir;
	settings.ExceptionCallback = ExceptionCallback;
	Rolky::HostInstance hostInstance;
	hostInstance.Initialize(settings);

	//Rolky::DotnetServices::RunMSBuild((exeDir.parent_path().parent_path() / "RolkyManaged.sln").string());
	...
	Rolky::ManagedObject testsInstance = testsType.CreateInstance();
	...
	Rolky::ManagedObject testsInstance2 = testsType2.CreateInstance();
