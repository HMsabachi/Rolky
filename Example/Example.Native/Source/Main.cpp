...
static void PrintStringIcall(Rolky::String InString)
{
	std::cout << std::string(InString) << std::endl;
}

static void NativeArrayIcall(Rolky::Array<float> InValues)
{
...
static Rolky::Array<float> ArrayReturnIcall()
{
	std::cout << "ArrayReturnIcall" << std::endl;
	return Rolky::Array<float>::New({ 10.0f, 5000.0f, 1000.0f });
}
...
int main([[maybe_unused]] int argc, char** argv)
{
	...
	auto arr = Rolky::Array<float>::New({ 5.0f, 0.0f, 10.0f, -50.0f });
	exampleInstance.InvokeMethod("ArrayDemo", arr);
	Rolky::Array<float>::Free(arr);
